...
class BSERGB_test_Dataset(data.Dataset):
    def __init__(self, opt):
        super(BSERGB_test_Dataset, self).__init__()
        self.opt = opt
        ...
        self.split = '3_TRAINING' if opt['phase'] == 'train' else '1_TEST'

        train_video_list = os.listdir(os.path.join(self.dataroot, '3_TRAINING'))
        test_video_list = os.listdir(
            os.path.join(self.dataroot, '1_TEST'))
        ...
        for video in video_list:
            ...
            event_frames = sorted(recursive_glob(rootdir=os.path.join(self.dataroot, self.split, video, 'events'), # NEED MODIFIED
                                                 suffix='.npz'), key=lambda x: int(x.split('.')[0]))
            n_sets = (len(frames) - 2) // (self.gt_setLength - 1)
    ...
    def __getitem__(self, index):
        ...
        events = np.concatenate([
            np.column_stack((
                event['timestamp'][:, np.newaxis],
                convert_and_fix_event_pixels(event['x'], FRAME_WIDTH - 1)[:, np.newaxis],
                convert_and_fix_event_pixels(event['y'], FRAME_HEIGHT - 1)[:, np.newaxis],
                event['polarity'][:, np.newaxis]
            )).astype(np.float32)
            for event in events
        ], axis=0)
